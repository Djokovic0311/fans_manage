	...
	<div class="searchDiv">
		<form action="<%=path %>/TempHandle" method="get">
			...
			<select name="condition">
				<option value='car_num'>活跃程度</option>
				<option value='entry_date'>上线日期</option>
				<option value='out_date'>下线日期</option>
			</select>
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				{
	               					out.print("<tr><td>"+obj[0]+"</td><td>"+obj[1]+"</td><td>"+obj[2]+"</td><td>"+obj[3]+" "+obj[4]+"</td><td>"+(outDate.equals("null null")?"未下线":outDate)+"</td><td>"+(obj[7].toString().equals("0.0")?"待计算":obj[7])+"</td><td><a href='"+path+"/Admin/TempEdit.jsp?temp_id="+obj[0]+"' class='a_edit'>计算</a><a href='"+path+"/TempHandle?type=1&temp_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a></td></tr>");
	               				}
	               				else{
	              	       	               		out.print("<tr><td>"+obj[0]+"</td><td>"+obj[1]+"</td><td>"+obj[2]+"</td><td>"+obj[3]+" "+obj[4]+"</td><td>"+(outDate.equals("null null")?"未下线":outDate)+"</td><td>"+(obj[7].toString().equals("0.0")?"待计算":obj[7])+"</td><td><a href='"+path+"/TempHandle?type=1&temp_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a>  <a onclick='return confirm(\"是否打印？\")'>打印</a></td></tr>");
	         	               		}
	                		}
                		}
