	...
	<div class="searchDiv">
		<form action="<%=path %>/SeatHandle" method="get">
			...
			<select name="condition">
			 	<option value='seat_id'>ID</option>
				<option value='seat_num'>职位编号</option>
				<option value='seat_section'>当前等级</option>
				<option value='seat_tag'>备注</option>
                <option value='seat_state'>当前状态</option>
			</select>
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				String result=obj[3].toString().equals("0")?"空缺":"占用";
